...
loadingOverlay.innerHTML = '<div class="loading-box"><div class="loading-spinner"></div><strong id="loadingText">Loading...</strong></div>';
...
function setLoading(isLoading, label = "Loading...") {
  ...
  loadingOverlay.hidden = networkBusyCount === 0;
  if (loadingText && networkBusyCount > 0) loadingText.textContent = label;
...
async function login(userKey, password) {
  ...
  if (!result.ok || !result.data?.token) {
    loginMessage.textContent = result.status === 0
      ? "API server is not reachable. Wait 30 seconds and try again."
      : result.status === 401
...
async function reports(forceRefresh = false) {
  ...
  if (!forceRefresh && cachedReportsLoaded) return cachedReports;
  if (reportsRequest && !forceRefresh) return reportsRequest;
  reportsRequest = requestJson("GET", "/api/reports", undefined, "Loading reports...")
    .then((source) => {
      cachedReports = normalizeReports(source || []);
      cachedReportsLoaded = true;
